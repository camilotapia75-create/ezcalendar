export default function DayCell({ day, currentMonth, isToday, flyers, onClick, onFlyerClick }) {
  return (
    <div
      onClick={onClick}
      className={[
    ...
    >
      ...
      <div className="space-y-1">
        {flyers.slice(0, 2).map((flyer) => (
          <button
            key={flyer.id}
            onClick={(e) => { e.stopPropagation(); onFlyerClick(flyer) }}
          ...
          >
            {flyer.imageData ? (
              ...
              <div className="w-full h-8 bg-indigo-500/20 rounded-lg flex items-center px-2 hover:bg-indigo-500/30 transition-colors">
                <span className="text-xs text-indigo-300 truncate">{flyer.title}</span>
              </div>
            )}
          </button>
        ))}
        {flyers.length > 2 && (
          <p className="text-[10px] text-gray-600 pl-0.5">+{flyers.length - 2} more</p>
        )}
        {currentMonth && flyers.length === 0 && (
