...
export default function DayView({ date, events, note, onClose, onAdd, onDelete, onPinStyleChange, onSaveNote }) {
  ...
  // Init canvas when entering write mode
  useEffect(() => {
  ...
  const onCanvasMove = (e) => {
    ...
    if (erasing) {
      ...
      ctx.beginPath()
      ctx.arc(pt.x * dpr, pt.y * dpr, penSize * 7 * dpr, 0, Math.PI * 2)
      ctx.fill()
      ctx.restore()
    } else {
      ctx.beginPath()
      ctx.moveTo(lastPtRef.current.x * dpr, lastPtRef.current.y * dpr)
  ...
  const commitText = () => {
    const text = pendingTextVal.trim()
    setPendingText(null)
    setPendingTextVal('')
    if (!text) return
    const canvas = canvasRef.current
    const ctx = canvas.getContext('2d')
    const dpr = Math.min(window.devicePixelRatio || 1, 2)
    const fontSize = penSize * 9
    ctx.font = `${fontSize * dpr}px Caveat, cursive`
    ctx.fillStyle = penColor
    ctx.fillText(text, pendingText.x * dpr, (pendingText.y + fontSize * 0.8) * dpr)
    hasChanges.current = true
  }
  ...
  const exitWriteMode = async () => {
    if (pendingTextVal.trim()) commitText()
    if (hasChanges.current && canvasRef.current) {
      ...
      await onSaveNote?.(dateKey, {
        ...
        drawing_data: hasPixels ? drawing : null,
      })
    }
    setWriteMode(false)
    setPendingText(null)
      ...
      <div className="fixed inset-0 z-50" style={{ background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(12px)' }} onClick={() => setSelectedEvent(null)}>
        <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: '5%', left: '5%', right: '5%', bottom: '5%', background: '#fffaee', border: '4px solid #1a1a1a', borderRadius: 8, boxShadow: '10px 10px 0 rgba(0,0,0,0.40)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flexShrink: 0, padding: '16px 20px 20px', borderTop: '2px solid #e9e0cc', background: '#fff' }}>
            {selectedEvent.title && <p style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 800, color: '#1a1a2e', lineHeight: 1.3 }}>{selectedEvent.title}</p>}
  ...
  return (
    <div className="fixed inset-0 z-50" style={{ background: 'rgba(0,0,0,0.70)', backdropFilter: 'blur(8px)' }} onClick={writeMode ? undefined : onClose}>
      <div
        ...
        style={{ position: 'absolute', top: '4%', left: '4%', right: '4%', bottom: '4%', background: '#fffaee', border: '5px solid #1a1a1a', borderRadius: 6, boxShadow: '10px 10px 0 rgba(0,0,0,0.40)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
      >
        {/* Header */}
        <div style={{ flexShrink: 0, padding: '14px 18px 12px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', borderBottom: '2px solid #e9e0cc', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
            {/* Write mode toggle */}
            <button
              onClick={() => writeMode ? exitWriteMode() : setWriteMode(true)}
              title={writeMode ? 'Done writing' : 'Write on this day'}
        ...
        {/* Board area — relative container for canvas overlay */}
        <div ref={boardRef} style={{ flex: 1, minHeight: 0, position: 'relative', overflow: writeMode ? 'hidden' : 'auto', WebkitOverflowScrolling: 'touch' }} onClick={() => setShowPinPicker(false)}>
          ...
          {/* Saved annotation — shown when NOT in write mode */}
          ...
          {/* Canvas overlay — active in write mode */}
          {writeMode && (
            <canvas
              ref={canvasRef}
              style={{ position: 'absolute', top: 0, left: 0, zIndex: 50, touchAction: 'none', cursor: writeTool === 'text' ? 'text' : erasing ? 'cell' : 'crosshair' }}
          ...
          {pendingText && (
            <textarea
              ...
              onBlur={commitText}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); commitText() } if (e.key === 'Escape') { setPendingText(null); setPendingTextVal('') } }}
              style={{
                position: 'absolute',
                left: pendingText.x,
                top: pendingText.y,
          ...
          {/* Write mode toolbar */}
          {writeMode && (
            <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', bottom: 14, left: '50%', transform: 'translateX(-50%)', zIndex: 55, background: 'rgba(255,254,240,0.97)', border: '2px solid #e9e0cc', borderRadius: 999, padding: '6px 14px', display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 20px rgba(0,0,0,0.18)', backdropFilter: 'blur(8px)', flexWrap: 'wrap', maxWidth: '92%', justifyContent: 'center' }}>
              {/* Draw / Text tool */}
              <div style={{ display: 'flex', borderRadius: 20, overflow: 'hidden', border: '1.5px solid #e9e0cc' }}>
                <button onClick={() => { setWriteTool('draw'); setPendingText(null) }}
                  style={{ padding: '3px 10px', fontSize: 13, border: 'none', cursor: 'pointer', background: writeTool === 'draw' ? '#7c3aed' : 'white', color: writeTool === 'draw' ? '#fff' : '#555', fontWeight: 700 }}>✏️ Draw</button>
                <button onClick={() => setWriteTool('text')}
                  style={{ padding: '3px 10px', fontSize: 13, border: 'none', cursor: 'pointer', background: writeTool === 'text' ? '#7c3aed' : 'white', color: writeTool === 'text' ? '#fff' : '#555', fontWeight: 700 }}>Aa Text</button>
              </div>
              {/* Colors */}
              {PEN_COLORS.map(c => (
                <button key={c} onClick={() => { setPenColor(c); setErasing(false) }}
                  style={{ width: penColor === c && !erasing ? 22 : 18, height: penColor === c && !erasing ? 22 : 18, borderRadius: '50%', background: c, border: penColor === c && !erasing ? '3px solid #1a1a1a' : '1.5px solid rgba(0,0,0,0.15)', cursor: 'pointer', flexShrink: 0, transition: 'all 0.1s', boxShadow: c === '#ffffff' ? 'inset 0 0 0 1px #ccc' : undefined }} />
              ))}
              {/* Sizes */}
              {[2, 4, 8].map(s => (
                <button key={s} onClick={() => { setPenSize(s); setErasing(false) }}
                  style={{ width: 26, height: 26, borderRadius: 6, border: penSize === s && !erasing ? '2px solid #7c3aed' : '1.5px solid #e5e5e5', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <div style={{ width: s + 2, height: s + 2, borderRadius: '50%', background: erasing ? '#ccc' : penColor }} />
                </button>
              ))}
              {/* Eraser */}
              <button onClick={() => setErasing(e => !e)}
                style={{ padding: '3px 9px', borderRadius: 20, border: erasing ? '2px solid #7c3aed' : '1.5px solid #e5e5e5', background: erasing ? '#ede9fe' : 'white', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: erasing ? '#7c3aed' : '#555' }}>Erase</button>
              {/* Clear */}
              <button onClick={clearCanvas}
                style={{ padding: '3px 9px', borderRadius: 20, border: '1.5px solid #fca5a5', background: '#fff1f2', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#ef4444' }}>Clear</button>
            </div>
          )}
                ...
                >
                  ...
                  <div style={{ flexShrink: 0, padding: '5px 7px 7px', background: '#fff', borderTop: '1px solid #f0ece0' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 3 }}>
                      ...
                      >{isEventOn(event.id) ? '🔔' : '🔕'}</button>
                    </div>
                  </div>
                  {!writeMode && (
                    <button onClick={(e) => { e.stopPropagation(); onDelete(event.id) }}
                      style={{ position: 'absolute', top: 4, right: 4, width: 18, height: 18, borderRadius: '50%', background: 'rgba(239,68,68,0.88)', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20, boxShadow: '0 2px 6px rgba(0,0,0,0.3)' }}
                    >✕</button>
                  )}
                </div>
              ))}
        ...
        {writeMode && (
          <div style={{ flexShrink: 0, padding: '10px 16px 14px', borderTop: '2px solid #e9e0cc', display: 'flex', justifyContent: 'center' }}>
            <button onClick={exitWriteMode} style={{ padding: '9px 32px', borderRadius: 12, background: 'linear-gradient(135deg,#7c3aed,#4f46e5)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>Done writing</button>
          </div>
        )}
      </div>
    </div>
