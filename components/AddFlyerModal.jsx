...
export default function AddFlyerModal({ date, onAdd, onClose, uploadImage }) {
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        <div className="flex items-center justify-between px-5 pt-14 pb-4">
          <button onClick={stopCamera} className="text-sm text-white/60 hover:text-white transition-colors">Cancel</button>
        ...
        <div className="flex justify-center pb-16 pt-8">
          <button onClick={capturePhoto} className="w-20 h-20 rounded-full border-4 border-white/70 flex items-center justify-center active:scale-95 transition-transform">
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center z-50" onClick={onClose}>
      <div className="w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-2xl" style={{ background: '#111113', border: '1px solid rgba(255,255,255,0.07)' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-center pt-3 pb-1 md:hidden">
          <div className="w-10 h-1 bg-white/10 rounded-full" />
        </div>
        <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <span className="font-semibold text-[15px]">
            {!imagePreview ? 'Scan a flyer' : analyzing ? 'Reading flyer…' : aiDetectedDate ? `📌 Placing on ${formatNice(eventDate)}` : 'Fill in details'}
          </span>
          <button onClick={onClose} className="text-zinc-600 hover:text-white text-xl leading-none transition-colors">&times;</button>
        </div>
        <div className="p-5">
          {!imagePreview ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={startCamera} className="flex flex-col items-center justify-center gap-2 h-28 rounded-2xl transition-all active:scale-[0.97]" style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.25)' }}>
                  <span className="text-3xl">📷</span>
                  <span className="text-xs font-medium text-violet-300">Camera</span>
                </button>
                <button type="button" onClick={() => fileRef.current.click()} className="flex flex-col items-center justify-center gap-2 h-28 rounded-2xl transition-all active:scale-[0.97]" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  <span className="text-3xl">🖼️</span>
                  <span className="text-xs font-medium text-zinc-400">Upload</span>
                </button>
              </div>
              <p className="text-center text-xs text-zinc-700">AI reads the flyer and places it on the right date</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="relative rounded-xl overflow-hidden" style={{ height: 208, background: '#000' }}>
                ...
                <>
                  <div className="relative">
                    <input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} required
                      ...
                      style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${aiDetectedDate ? 'rgba(124,58,237,0.45)' : 'rgba(255,255,255,0.07)'}` }} />
                    {aiDetectedDate && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-violet-400 font-bold tracking-widest uppercase">AI</span>}
                  </div>
                  <input type="text" placeholder="Event title" value={title} onChange={(e) => setTitle(e.target.value)}
                    ...
                    style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${title ? 'rgba(124,58,237,0.35)' : 'rgba(255,255,255,0.07)'}` }} />
                  <div className="grid grid-cols-2 gap-3">
                    <input type="text" placeholder="Location" value={location} onChange={(e) => setLocation(e.target.value)}
                      className="w-full rounded-xl px-4 py-2.5 text-sm placeholder-zinc-700 focus:outline-none transition-colors"
                      style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${location ? 'rgba(124,58,237,0.35)' : 'rgba(255,255,255,0.07)'}` }} />
                    <input type="text" placeholder="Time" value={timeStr} onChange={(e) => setTimeStr(e.target.value)}
                      className="w-full rounded-xl px-4 py-2.5 text-sm placeholder-zinc-700 focus:outline-none transition-colors"
                      style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${timeStr ? 'rgba(124,58,237,0.35)' : 'rgba(255,255,255,0.07)'}` }} />
                  </div>
