...
// First-run demo banner — explains that the populated feed is sample content and
// offers to clear it. Shown only while the client-side samples are on screen.
  ...
  return (
    <div style={{ marginBottom: 26 }}>
      ...
      <div className="hide-scroll" style={{ display: 'flex', gap: 10, overflowX: 'auto', WebkitOverflowScrolling: 'touch', padding: '0 16px 4px' }}>
        {items.map((s, i) => (
          <div key={i} onClick={() => onOpen?.(s)} style={{ flexShrink: 0, width: 128, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', flexDirection: 'column', cursor: 'pointer' }}>
            <div style={{ position: 'relative', paddingTop: '52%', background: s.image ? '#000' : 'linear-gradient(150deg, #d4f560, #8fbf2e)' }}>
